...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    current12_bricklet.set_debounce_period(10000);

    //Create listener for current reached events.
    let current_reached_listener = current12_bricklet.get_current_reached_receiver();
    // Spawn thread to handle received events. This thread ends when the current12_bricklet
    ...
    current12_bricklet.set_current_callback_threshold('>', 5 * 1000, 0);
